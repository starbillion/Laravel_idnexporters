...
@section('title', __('faq.post_data.add'))

@section('content')
<div class="block block-bordered">
	<form action="{{ route('admin.faq.post.store') }}" method="post">
		{{ csrf_field() }}
			...
			<div class="row">
				<div class="col-md-6">
					<div class="form-group{{ $errors->has('question') ? ' is-invalid' : '' }}">
						...
						<input type="text" class="form-control" id="field-question" name="question" value="{{ old('question') }}">
						<div class="invalid-feedback">{{ $errors->first('question') }}</div>
					</div>
				</div>
				<div class="col-md-6">
					<div class="form-group{{ $errors->has('category_id') ? ' is-invalid' : '' }}">
						<label for="field-category_id">{{ __('faq.category') }}</label>
				...
				<div class="col-md-12">
					<div class="form-group{{ $errors->has('answer') ? ' is-invalid' : '' }}">
						...
						<textarea id="field-answer" class="form-control" name="answer" rows="6">{{ old('answer') }}</textarea>
